...
/*   Updated: 2025/02/14 23:26:33 by asajed           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	my_exec(t_pipex *data, int var)
{
	int	fd_in;
	...
	fd_in = data->in_fd;
	while (var < data->cmd_count)
	{
		if (pipe(data->fd) == -1)
			ft_error(strerror(errno), data, 1, 1);
		data->pids[var - 1] = fork();
		if (data->pids[var - 1] == -1)
			ft_error(strerror(errno), data, 1, 1);
		if (data->pids[var - 1] == 0)
			child_process(data, fd_in, var, data->fd);
		ft_close(&fd_in);
		if (var < data->cmd_count - 1)
		{
			ft_close(&data->fd[1]);
			fd_in = data->fd[0];
		}
